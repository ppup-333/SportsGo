...
<body>
	...
	<form action="bbsdiscussion.r" method="get">
		<input type="search" id="search" name="search" autocomplete="off" placeholder="輸入查詢字串...">
		<input type="image" src="images/magnifier.png" width="20" height="20">
	</form>
		...
		<table border="1">
			...
			<c:if test="${bbsList.size() > 0}">
				<c:forEach var="row" items="${bbsList}">
					<tr>
						<td>${row.bbsType.typeName}</td>
						<td>${row.bbsBuilder}</td>
						<td><a href="<c:url value='bbsSelect' />?bbsId=${row.bbsId}">
								<div>
									<div>
										<p>${row.bbsTitle}</p>
									</div>
						...
						<%-- 					<td>${row.bbsTitle}<br>${row.bbsMessage}</td> --%>
						<td><fmt:formatDate value="${row.bbsSetupTime}"
								pattern="YYYY-MM-dd" /><br>
								<fmt:formatDate value="${row.bbsSetupTime}"
								pattern="HH:mm" /></td>
